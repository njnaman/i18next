...
describe('v1.11.1 initialisation', () => {
  ...
  describe('with passed in resource set', () => {
    ...
    it('it should provide passed in resources for translation', () => {
      expect(i18n.t('simple_en-US')).toBe('ok_from_en-US');
      expect(i18n.t('simple_en')).toBe('ok_from_en');
      expect(i18n.t('simple_dev')).toBe('ok_from_dev');
    });
  });
  ...
  describe('loading from server with static route', () => {
    ...
    it('it should provide loaded resources for translation', () => {
      ...
      expect(i18n.t('xhr_simple_en-US')).toBe('xhr_ok_from_en-US');
      expect(i18n.t('xhr_simple_en')).toBe('xhr_ok_from_en');
      expect(i18n.t('xhr_simple_dev')).toBe('xhr_ok_from_dev');
    });
  });
  ...
  describe('advanced initialisation options', () => {
    describe('setting fallbackLng', () => {
      ...
      it('it should provide passed in resources for translation', () => {
        expect(i18n.t('simple_en-US')).toBe('ok_from_en-US');
        expect(i18n.t('simple_en')).toBe('ok_from_en');
        expect(i18n.t('simple_dev1')).toBe('ok_from_dev1');
      });
    });
    ...
    describe('multiple fallbackLng', () => {
      ...
      it('it should provide passed in resources for translation', () => {
        ...
        // in one
        expect(i18n.t('simple_dev1')).toBe('ok_from_dev1');
        expect(i18n.t('simple_dev2')).toBe('ok_from_dev2');
        // in both
        expect(i18n.t('simple_dev')).toBe('ok_from_dev1');
      });
    });
    ...
    describe('adding resources after init', () => {
      ...
      describe('resources', () => {
        ...
        it('it should provide passed in resources for translation', () => {
          expect(i18n.t('some.deep.thing')).toBe('ok_from_en-US');
        });
        ...
        describe('multiple resources', () => {
          ...
          it('it should add the new namespace to the namespace array', () => {
            expect(i18n.t('some.other.deep.thing')).toBe('ok_from_en-US_1');
            expect(i18n.t('some.other.deep.deeper.thing')).toBe('ok_from_en-US_2');
          });
        });
      });
      ...
      describe('bundles', () => {
        ...
        describe('with using deep switch', () => {
          ...
          it('it should add the new namespace to the namespace array', () => {
            expect(i18n.t('deep.simple_en-US_1')).toBe('ok_from_en-US_1');
            expect(i18n.t('deep.simple_en-US_2')).toBe('ok_from_en-US_2');
          });

          it('it should not overwrite any existing entries if the overwrite switch is off', () => {
            expect(i18n.t('deep.simple_en-US_3')).toBe('ok_from_en-US_3');
          });
        });
        ...
        describe('with using deep switch and overwrite switch', () => {
          ...
          it('it should overwrite any existing entries if the overwrite switch is on', () => {
            expect(i18n.t('deep.simple_en-US_3')).toBe('ok_from_en-US_3-overwrite');
          });
        });
    ...
    describe('getting resources after init', () => {
      ...
      it('it should return resources for existing bundle', () => {
        ...
        expect(devTranslation.test).toBe('ok_from_dev');
        expect(enTranslation.test).toBe('ok_from_en');
        expect(enUSTranslation.test).toBe('ok_from_en-US');
      });

      it('it should return empty object for non-existing bundle', () => {
        const nonExisting = i18n.getResourceBundle('en-GB', 'translation');
        expect(Object.keys(nonExisting).length).toBe(0);
      });
      ...
      it('it should return a clone of the resources', () => {
        ...
        expect(enTranslation.test).toBe('ok_from_en_changed');
        expect(resStore.en.translation.test).toBe('ok_from_en');
      });
    });
    ...
    describe('removing resources after init', () => {
      ...
      it('it should remove resources', () => {
        expect(i18n.t('test')).toBe('ok_from_en');
      });
    });
    ...
    describe('setting load', () => {
      describe('to current', () => {
        ...
        it('it should provide loaded resources for translation', () => {
          expect(i18n.t('simple_en-US')).toBe('ok_from_en-US');
          expect(i18n.t('simple_en')).not.toBe('ok_from_en');
          expect(i18n.t('simple_dev')).toBe('ok_from_dev');
        });
      });
      ...
      describe('to unspecific', () => {
        ...
        it('it should provide loaded resources for translation', () => {
          expect(i18n.t('simple_en-US')).not.toBe('ok_from_en-US');
          expect(i18n.t('simple_en')).toBe('ok_from_en');
          expect(i18n.t('simple_dev')).toBe('ok_from_dev');
        });

        it('it should return unspecific language', () => {
          expect(i18n.lng()).toBe('en');
        });
      });
    });
    ...
    describe('with fallback language set to false', () => {
      ...
      it('it should provide loaded resources for translation', () => {
        expect(i18n.t('simple_en-US')).toBe('ok_from_en-US');
        expect(i18n.t('simple_en')).toBe('ok_from_en');
        expect(i18n.t('simple_dev')).not.toBe('ok_from_dev');
      });
    });
    ...
    describe('with namespace', () => {
      describe('with one namespace set', () => {
        ...
        it('it should provide loaded resources for translation', () => {
          expect(i18n.t('simple_en-US')).toBe('ok_from_special_en-US');
          expect(i18n.t('simple_en')).toBe('ok_from_special_en');
          expect(i18n.t('simple_dev')).toBe('ok_from_special_dev');
        });
      });
      ...
      describe('with more than one namespace set', () => {
        ...
        it('it should provide loaded resources for translation', () => {
          ...
          // ns prefix
          expect(i18n.t('ns.common:simple_en-US')).toBe('ok_from_common_en-US');
          expect(i18n.t('ns.common:simple_en')).toBe('ok_from_common_en');
          expect(i18n.t('ns.common:simple_dev')).toBe('ok_from_common_dev');

          // ns in options
          expect(i18n.t('simple_en-US', { ns: 'ns.common' })).toBe('ok_from_common_en-US');
          expect(i18n.t('simple_en', { ns: 'ns.common' })).toBe('ok_from_common_en');
          expect(i18n.t('simple_dev', { ns: 'ns.common' })).toBe('ok_from_common_dev');
        });
        ...
        describe('and fallbacking to default namespace', () => {
          ...
          it('it should fallback to default ns', () => {
            ...
            expect(i18n.t('ns.common:simple_en-US')).toBe('ok_from_en-US');
            expect(i18n.t('ns.common:simple_en')).toBe('ok_from_en');
            expect(i18n.t('ns.common:simple_dev')).toBe('ok_from_dev');

            // ns in options
            expect(i18n.t('simple_en-US', { ns: 'ns.common' })).toBe('ok_from_en-US');
            expect(i18n.t('simple_en', { ns: 'ns.common' })).toBe('ok_from_en');
            expect(i18n.t('simple_dev', { ns: 'ns.common' })).toBe('ok_from_dev');
          });
        });
        ...
        describe('and fallbacking to set namespace', () => {
          ...
          it('it should fallback to set fallback namespace', () => {
            expect(i18n.t('ns.common:simple_fallback')).toBe('ok_from_fallback');
          });
        });
        ...
        describe('and fallbacking to multiple set namespace', () => {
          ...
          it('it should fallback to set fallback namespace', () => {
            expect(i18n.t('ns.common:simple_fallback')).toBe(
              'ok_from_fallback1',
            ); /* first wins */
            expect(i18n.t('ns.common:simple_fallback1')).toBe('ok_from_fallback1');
            expect(i18n.t('ns.common:simple_fallback2')).toBe('ok_from_fallback2');
          });
      ...
      describe('with reloading additional namespace', () => {
        describe('without using localStorage', () => {
          ...
          describe('and fallbackToDefaultNS turned on', () => {
            ...
            it('it should fallback to default namespace', () => {
              expect(i18n.t('ns.special:test.fallback_en')).toBe('ok_from_common_en-fallback');
              expect(i18n.t('ns.special:test.fallback_dev')).toBe(
        ...
        //       expect(spy.callCount).toBe(9); // en-US, en, de-DE, de, fr, dev * 3 namespaces (translate, common, special)
        //     });
        //
    ...
    //
    //   it('it should load language', function() {
    //     expect(spy.callCount).toBe(3); // en-US, en, de-DE, de, fr, dev
    //   });
    //
    ...
    //
    //     it('it should not reload language', function() {
    //       expect(spy.callCount).toBe(0); // de-DE, de, fr, dev
    //     });
    //
    ...
    //         expect(spy.callCount).toBe(1); // de-DE, de, fr, dev
    //       });
    //
    //     });
    //
    ...
    describe("using function provided in callback's argument", () => {
      ...
      it('it should provide loaded resources for translation', () => {
        expect(tCallback('simple_en-US')).toBe('ok_from_en-US');
        expect(tCallback('simple_en')).toBe('ok_from_en');
        expect(tCallback('simple_dev')).toBe('ok_from_dev');
      });
    });
    ...
    describe('with language whitelist', () => {
      ...
      it('should degrade UNwhitelisted 2-part lang code (en-US) to WHITELISTED 1-part (en)', async () => {
        await i18n.init(
          ...
          () => {
            expect(i18n.lng()).toBe('en');
            expect(i18n.t('string_one')).toBe('good_en');
          },
        );
      });
      ...
      it('should NOT degrade WHITELISTED 2-part lang code (zh-CN) to UNwhitelisted 1-part (en)', async () => {
        ...
        await i18n.init(
          ...
          () => {
            ...
            expect(i18n.t('string_one')).toBe('good_zh-CN');
          },
        );
      });
    });
