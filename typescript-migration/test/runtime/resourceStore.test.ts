  ...
  describe('resource manipulation', () => {
    ...
    describe('can add resources', () => {
      ...
      it('it adds resources by addResource', () => {
        ...
        expect(rs.getResource('de', 'translation', 'test')).toBe('test');

        // dotted key
        rs.addResource('de', 'translation', 'nest.test', 'test_nest');
        expect(rs.getResource('de', 'translation', 'nest.test')).toBe('test_nest');

        // using first param fullDot
        rs.addResource('de.translation.nest.fullDot', 'test_fullDot');
        expect(rs.getResource('de.translation.nest.fullDot')).toBe('test_fullDot');

        // setting object
        rs.addResource('de.translation.nest.object', { something: 'deeper' });
        expect(rs.getResource('de.translation.nest.object.something')).toBe('deeper');

        // getting object
        expect(rs.getResource('de.translation.nest.object')).toEqual({ something: 'deeper' });
      });
      ...
      it('it should also addResources nested in wrong way', () => {
        ...
        expect(rs.getResource('fr.translation.a.key')).toEqual('first');
        expect(rs.getResource('fr.translation.a.key.deep')).toEqual('second');
        expect(rs.getResource('fr.translation.a.key.deep.deeper')).toEqual('third');
        expect(rs.getResource('fr.translation.a.key.deeper')).toEqual('fourth');
        expect(rs.getResource('fr.translation.b.k')).toEqual('fifth');
      });
    });
    ...
    describe('can extend resources bundle', () => {
      ...
      it('it adds resources by addResourceBundle', () => {
        ...
        expect(rs.getResource('en', 'translation')).toEqual({ something: 'deeper', test: 'test' });

        // dotty
        rs.addResourceBundle('en.translation', { something1: 'deeper1' });
        expect(rs.getResource('en.translation')).toEqual({
          something: 'deeper',
          something1: 'deeper1',
          test: 'test',
        });
      ...
      it('without polluting the prototype by __proto__', () => {
        ...
        expect({}.vulnerable).toEqual(undefined);
      });
